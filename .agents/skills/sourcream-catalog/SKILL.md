...
Only add movies the user selects. There is no automatic folder scanning or importing. Media paths passed to the catalog CLI are relative to `MOVIES_ROOT` (default `D:/Movies`); preserve the source files and their folders. Generated artwork and UTF-8 WebVTT files go to `ASSETS_ROOT`, separate from the originals.
...
2. Run `npm run movie -- add "relative/movie.mp4" --id <id> --title "Title" --year <year>` with any verified `--director`, repeated `--genre`, `--synopsis`, and repeated `--subtitle "relative-file|language|label"` options. Do not invent uncertain metadata; verify it or ask when necessary. Never download films or subtitles as part of adding a catalog entry.
3. Existing IDs require `--replace`, which updates that entry and replaces its subtitle associations. The command validates files and codecs, converts UTF-8 SRT to WebVTT, and generates 320px player preview frames every 10 seconds. Existing preview derivatives are retained when the registered video is unchanged.
...
Catalog data lives only in SQLite; do not create or maintain separate manifest files. Browser local storage is exclusively for device-local playback progress and settings. This is a LAN app; publishing media or opening WAN access is a separate task.
